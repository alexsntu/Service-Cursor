# Карточка товара, список услуг и импорт — новый сайт iRepair (CS-Cart + UniTheme2)
...
| `blocks/product_list_templates/irepair_service_list.tpl` (+ `default_params/…`) | Вид списка **«iRepair — Список услуг»** (копия «Компактного списка») |
| `blocks/list_templates/irepair_service_list.tpl` | Вёрстка списка: прайс как на старом сайте (название / «от» цена + время ремонта / «Заказать ремонт») |
...
- ⚠️ **Модуль «Баннеры категорий» (`ab__category_banners/func.php`) пропатчен**: без патча любая категория
  со своим видом списка падает с ошибкой БД. После обновления модуля — повторить патч
  (ветка `default:` → `$image_type='S'; $layout_name='short_list';`). Нужно и на боевом сайте.
