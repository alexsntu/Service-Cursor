...
- **Проектное ограничение (Service-Cursor/irepair.ru):** сообщайте о пробелах в схеме как о находках аудита; саму разметку не генерировать — см. предупреждение в `geo-schema/SKILL.md` (Step 6) про обязательный `@graph`-регламент `seo--for-service-html-block.mdc`
