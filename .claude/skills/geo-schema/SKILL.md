...
> **Источник:** адаптировано из [TheSmokeDev/geo-skills](https://github.com/TheSmokeDev/geo-skills) (MIT License, см. `LICENSE` в этой папке). Скопирован только `SKILL.md`, без Python-скриптов/hooks/CRM-инструментов исходного репозитория.
...
> **⚠️ Проектное ограничение (Service-Cursor / irepair.ru):** этот проект уже генерирует свой `@graph` (LocalBusiness + WebPage + FAQPage) по строгому регламенту `.cursor/rules/seo--for-service-html-block.mdc` через скилл `seo-service-page-builder` — с фиксированными брендовыми константами (телефон, адрес, `priceRange: "₽₽"`, запрет конкретных цен и т.д.). **В этом проекте Step 6 не использовать для генерации новой разметки** — только для аудита (Steps 1–5) уже существующих на сайте JSON-LD-блоков. Если аудит находит расхождения с `seo--for-service-html-block.mdc`, сообщайте о них как о находках, а саму разметку правьте через `seo-service-page-builder`/`seo-service-page-workflow`, а не через шаблоны ниже.
