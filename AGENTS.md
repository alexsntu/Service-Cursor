...
### Shared project memory (user instruction, 2026-09-06)

- The canonical project memory is Claude Code's directory: `/Users/a0000/.claude/projects/-Users-a0000-----------Cursor-Service-Cursor/memory/`.
- Before project work, read `MEMORY.md` there and the relevant linked files. The user maintains the project through Claude Code; do not create a competing Codex memory.
- Save all meaningful progress, decisions, verified results, unresolved issues, and next steps back into the relevant existing files there; keep `MEMORY.md` current. Distinguish observations from assumptions and correct disproved entries without erasing useful history.
- Never copy credentials into this repository or reports. Access the external credentials file only as needed. Follow filesystem approval requirements when writing the external memory.
